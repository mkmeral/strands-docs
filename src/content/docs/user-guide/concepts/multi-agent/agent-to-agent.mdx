...
`A2AAgent` class is not currently supported in the TypeScript SDK.
:::
...
## Accessing A2A request context

When a client sends a message to your A2A server, the incoming request carries context—the task ID, caller metadata, configuration, and related tasks. By default, this context is handled by the A2A protocol layer and your agent's tools never see it. However, there are situations where your tools and hooks need this information.

For example, imagine you're building a multi-tenant A2A agent where different callers pass metadata to identify their organization, priority level, or feature flags. Without access to the request context, your tools can't customize their behavior based on who is calling or why.

The A2A executor passes the full `RequestContext` object through the agent's `invocation_state` under the `a2a_request_context` key. This makes it available to tools through `ToolContext.invocation_state` and to hooks through event `invocation_state`.

:::note
The `a2a_request_context` key is only present when the agent is invoked through the A2A server. For non-A2A invocations (direct `agent()` calls), this key doesn't exist. Always use `.get()` to check for it.
:::

### Accessing request context in tools

Tools that use `ToolContext` can read the request context from `invocation_state`:

```python
from strands import tool, ToolContext

@tool(context=True)
def process_request(input: str, tool_context: ToolContext) -> str:
    """Process a request with A2A context awareness.
    ...
    if a2a_context:
        ...
        return f"Processed '{input}' for tenant '{tenant}' (task: {task_id})"

    # Graceful fallback for non-A2A invocations
    return f"Processed '{input}'"
```

### Accessing request context in hooks

[Hooks](../agents/hooks.mdx) receive `invocation_state` on their event objects, allowing you to observe or modify behavior based on A2A request context:
...
def log_a2a_context(event: BeforeToolCallEvent) -> None:
    """Log A2A request details before each tool call."""
    a2a_context = event.invocation_state.get("a2a_request_context")

    if a2a_context:
        logger.info(
            "A2A request — task: %s, context: %s, metadata: %s",
            a2a_context.task_id,
            a2a_context.context_id,
            a2a_context.metadata,
        )
```

Register this hook on the agent that powers your A2A server:
...
agent = Agent(
    ...
    description="An agent that logs A2A request context.",
    tools=[process_request],
    callback_handler=None,
)
agent.add_hook(log_a2a_context, BeforeToolCallEvent)

a2a_server = A2AServer(agent=agent)
a2a_server.serve()
```

### Available RequestContext fields

The `a2a_request_context` value is a `RequestContext` object from the A2A SDK. The following table describes the available properties.

| Property | Type | Description |
|----------|------|-------------|
| `task_id` | `str` or `None` | Identifier for the current task |
| `context_id` | `str` or `None` | Identifier for the conversation context |
| `current_task` | `Task` or `None` | The current task object being processed |
| `metadata` | `dict` | Caller-provided metadata from the request |
| `configuration` | `MessageSendConfiguration` or `None` | Configuration from the request parameters |
| `call_context` | `ServerCallContext` or `None` | Server-level call context |
| `related_tasks` | `list[Task]` | Tasks related to the current request |
| `requested_extensions` | `set[str]` | Protocol extensions requested by the client |
| `message` | `Message` or `None` | The incoming A2A message |
